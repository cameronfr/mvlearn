...
class CTClassifier(object):
    ...
    def fit(self, Xs, y, p=None, n=None, unlabeled_pool_size=75, num_iter=50, y_train_full=None, X1_test=None, X2_test=None, y_test=None):
        ...
        self.u_pool_size = unlabeled_pool_size
        self.num_iter_ = num_iter

        print(self.n_)
        print(self.p_)

        assert(self.p_ > 0 and self.n_ > 0 and self.num_iter_ > 0 and self.u_pool_size > 0)
        ...
        #this is U' in paper
        unlabeled = U[-min(len(U), self.u_pool_size):]
        ...
        while it < self.num_iter_ and U:
            ...
            self.h1_.fit(X1[L], y[L])
            self.h2_.fit(X2[L], y[L])
            print(len(L))
            ###y_test_new = y_test[unlabeled]

            y1_prob = self.h1_.predict_log_proba(X1[unlabeled])
            y2_prob = self.h2_.predict_log_proba(X2[unlabeled])
...
#                         print("h2 guessed 1 actually " + str(y_test_new[i]))
    ...
    def predict(self, Xs):
        ...
        y1 = self.h1_.predict(X1)
        y2 = self.h2_.predict(X2)

        proba_supported = self.supports_proba(self.h1_, X1[0]) and self.supports_proba(self.h2_, X2[0])
        ...
        for i, (y1_i, y2_i) in enumerate(zip(y1, y2)):
            if y1_i == y2_i:
                ...
                y1_probs = self.h1_.predict_proba([X1[i]])[0]
                y2_probs = self.h2_.predict_proba([X2[i]])[0]
    ...
    def predict_proba(self, Xs):
        ...
        y1_proba = self.h1_.predict_proba(X1)
        y2_proba = self.h2_.predict_proba(X2)
